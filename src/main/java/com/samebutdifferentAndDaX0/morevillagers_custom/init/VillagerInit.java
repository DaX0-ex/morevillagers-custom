package com.samebutdifferentAndDaX0.morevillagers_custom.init;
...
// REGISTER WORKSTATIONS AND PROFESSIONS
    ...
    // CREATE INVENTOR WORKSTATION AND PROFESSION
    public static final RegistryObject<PointOfInterestType> INVENTOR_POI = POINT_OF_INTEREST_TYPES.register("inventor",
            () -> new PointOfInterestType("inventor", PointOfInterestType.getBlockStates(BlockInit.INVENTOR_TABLE.get()), 1, 1));
    public static final RegistryObject<VillagerProfession> INVENTOR = VILLAGER_PROFESSIONS.register("inventor",
            () -> new VillagerProfession("inventor", INVENTOR_POI.get(), ImmutableSet.of(), ImmutableSet.of(), SoundEvents.VILLAGER_WORK_CARTOGRAPHER));
    public static void registerInventorPOI() {
        try {
            ObfuscationReflectionHelper.findMethod(PointOfInterestType.class, "registerBlockStates", PointOfInterestType.class).invoke(null, OCEANOGRAPHER_POI.get());
        } catch (InvocationTargetException | IllegalAccessException e) {
            e.printStackTrace();
    ...
    // INVENTOR TRADES
        ...
        VillagerTrades.ITrade[] inventorLevel1 = new VillagerTrades.ITrade[]{
                new VillagerTrades.ItemsForEmeraldsTrade(AllItems.COPPER_INGOT.get(), 16, 6, 20, 8),
                new VillagerTrades.ItemsForEmeraldsTrade(AllItems.ZINC_INGOT.get(), 16, 6, 20, 8),
                new VillagerTrades.ItemsForEmeraldsTrade(AllItems.ANDESITE_ALLOY.get(), 3, 8, 16, 8),
                new VillagerTrades.EmeraldForItemsTrade(Items.ANDESITE, 8, 10, 12),
        };
        VillagerTrades.ITrade[] inventorLevel3 = new VillagerTrades.ITrade[]{
                new VillagerTrades.ItemsForEmeraldsTrade(AllBlocks.SAIL.get(), 4, 8, 8, 8),
                new VillagerTrades.ItemsForEmeraldsTrade(AllBlocks.BELT.get(), 12, 1, 8, 8),
        };
        VillagerTrades.ITrade[] inventorLevel4 = new VillagerTrades.ITrade[]{
                new VillagerTrades.ItemsForEmeraldsTrade(AllBlocks.COGWHEEL.get(), 8, 4, 12, 9),
                new VillagerTrades.ItemsForEmeraldsTrade(AllBlocks.LARGE_COGWHEEL.get(), 8, 4, 12, 9),
        };
        VillagerTrades.ITrade[] inventorLevel6 = new VillagerTrades.ITrade[]{
                new VillagerTrades.ItemsForEmeraldsTrade(AllBlocks.BLAZE_BURNER.get(), 64, 1, 72, 16),
        };
        VillagerTrades.TRADES.put(INVENTOR.get(),toIntMap(ImmutableMap.of(1,inventorLevel1,3,inventorLevel3,4,inventorLevel4,5,inventorLevel6)));
}
